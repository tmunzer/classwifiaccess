...
module.exports = function (router, isAuthenticated, isAtLeastOperator) {
    ...
    router.get("/conf/classroom/", isAuthenticated, isAtLeastOperator, function (req, res, next) {
    ...
    /* GET User Edit page. */
    router.get('/conf/classroom/edit', isAuthenticated, isAtLeastOperator, function (req, res, next) {
        var classroomIdToEdit = req.query.id;
    ...
    router.post("/conf/classroom/edit", isAuthenticated, isAtLeastOperator, function (req, res, next) {
        var classroomIdToEdit = req.query.id;
    ...
    router.get("/conf/classroom/new/", isAuthenticated, isAtLeastOperator, function (req, res, next) {
        ...
        Device.getAll(filterString, function (err, deviceList) {
            if (err) Error.render(err, "conf", req, res);
            else {
                School.getAll(filterString, function (err, schoolList) {
                    if (err) Error.render(err, "conf", req, res);
                    // render the page
                    res.render('conf_classroomEdit', {
                        user: req.user,
